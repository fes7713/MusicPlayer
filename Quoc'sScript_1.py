...
class Ui_Form(QtWidgets.QWidget):
    ...
    def __init__(self, Form):
        ...
        self.PreviousPath = []
        self.NextPath = []
        self.filepath = "J.Geco - Chicken Song (320  kbps).mp3"
        ##############
        # Set Icons ##
        ##############
    ...
    def button_openfile(self):
        ...
        self.filepath, _ = QFileDialog.getOpenFileName(self, 'Open file', 'c:\\', "Audio files (*.wav)")
        self.filepath = os.path.abspath(self.filepath)

        self.PlaySongAtPath(self, self.filepath)
    ...
    def button_next(self):
        ...
        if (len(self.NextPath) == 0):
            ...
            while (True):
                ...
                newfile = entries.pop(random.randrange(0, len(entries)))
                print(len(entries))
                print(newfile)
                print(self.filepath)
                if newfile == self.filepath and len(entries) == 0:
                    print(newfile)
                    NewPath = os.path.dirname(self.filepath)
                    NewPath += newfile
                    break
                elif newfile == self.filepath and len(entries) != 0: continue
                check = newfile[len(newfile) - 3: len(newfile)]
            ...
            NewPath = self.NextPath.pop(len(self.NextPath) - 1)

        self.PreviousPath.append(self.filepath)
    ...
    def button_previous(self):
        ...
        print(len(self.PreviousPath))
        if len(self.PreviousPath) > 0:
            NewPath = self.PreviousPath.pop(len(self.NextPath) - 1)
            print(NewPath)
        self.NextPath.append(self.filepath)
        self.filepath = NewPath
        self.PlaySongAtPath()

    def PlaySongAtPath(self):
        self.qsound = QSound(self.filepath)
